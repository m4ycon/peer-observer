...
async fn check(
    args: Vec<&str>,
    test_setup: fn(&corepc_node::Client, &corepc_node::Client),
...
async fn test_integration_logextractor_saw_new_header() {
    ...
    check(
        vec![],
        |_, node2| {
            // it needs 2 blocks to be sent so one of them triggers the
            // SawNewHeaderLog, maybe it also relates to the compact block flow
...
async fn test_integration_logextractor_compact_block_reconstruction() {
    ...
    check(
        ...
        |_, node2| {
            node2.generate_to_address(1, &REGTEST_ADDRESS).unwrap(); // this block will trigger SENDCMPCT trade between nodes
            node2.generate_to_address(1, &REGTEST_ADDRESS).unwrap(); // this block will trigger the actual CompactBlockReconstructedLog
        },
        |event| match event {
            PeerObserverEvent::LogExtractor(r) => {
